...
## D4 运行组状态恢复与媒体/前端回归（2026-09-24）

详见 `docs/qa/003-recovery-next-20260924.md`：Supervisor 状态文件的旧组删除竞态已修复并在 D4 API restart 验证新组 7/7、旧组 0 残留且文件持续存在（T138）；项目自有 PowerPointHost 的超时退出已验证，但用户 Office 的持续归属仍未解决（T133/T143）。

视频同源重开、循环后实体画面与非循环自然结束状态已在 D4 短片上复测，迟到结束事件有自动 fencing 回归（T136/T139）；音频一次暂停后的 WASAPI 输出为 0 字节（T140）；文件夹空名和编辑菜单的真实浏览器红/绿复测通过（T142）。本机非 Physical .NET 236/236、前端 40/40、类型检查与 Web 构建通过。PDF 单独开关及 40 次重复开关未复现上轮文件占用，T141 未关闭；T115/T116/T129 与实流、预案音量门禁仍待执行。

2026-09-25 经用户授权，已仅关闭 D4 特定旧 PowerPoint 自动化实例 PID 49584；正常 `Quit()` 未完成，精确身份复核后强制结束，结束时全机 PowerPoint 进程为 0。随后本机以太网物理链路断开，尚未执行 PPTX 再打开和关闭的 D4 实测，T133/T143 不勾选。过程与恢复后的首要核对项见 `docs/qa/003-office-interop.md`。
