...
- D4 多源冒烟发现并修复 WebView2 脱离视觉树初始化卡死、PowerPoint 操作丢失 STA 上下文、音频异步启动时误报持续加载、受控停机后旧执行租约阻塞新命令，以及前端 `prev` 与后端 `previous` 不一致；非 Physical .NET 测试 226/226 通过。
- 现场上传并打开图片、视频、PDF、网页、音频和 PPTX，另验证播放主机本地路径；交互桌面截图确认图片、视频帧、PDF、网页与 PPT 实际显示。MediaMTX 短时 RTSP 推拉流通过，但应用尚无直播 URL 创建入口；视频自然结束后 API 仍显示 `playing`，仍待后续修复。未执行 60 分钟混合门禁。
...
- `.ps1` 必须 UTF-8 带 BOM 这条约定此前只体现在文件头（4 个脚本都是 `EF BB BF`），没有任何文档写它。Windows 自带的 `powershell.exe` 是 5.1，对无 BOM 的脚本按当前 ANSI 代码页解码，中文注释与提示会乱码甚至解析失败——开发机与现场工作站都吃这条，只有另装的 `pwsh` 7 才默认按 UTF-8 读。现补为 `STYLE.md` §8「PowerShell 脚本」，同时收进「面向现场的报错写法、非持久改动（`store=active`）、动作幂等」这几条新脚本已经遵循的实践。

### 视频墙下发补上真实 TCP 的回环实测

- 把 `192.168.5.101~150` 挂成本机网卡别名（`runtime-dotnet/scripts/videowall-loopback.ps1`，`store=active`，重启即消失），假节点在真实端口 `4830` 上收包，于是原本「仅代码复核」的传输层行为第一次有了实测数字：**全节点 200 包 224~240 ms**、**单个节点掉线或拒连 12.4~12.6 s 后中止**（正好是 5 次 2 秒超时加 0.2/0.4/0.8/1.0 秒退避）、**400 ms 取消在 409 ms 原样抛出且零日志**。这同时给 SC-005 的「3 秒常态 / 90 秒最坏」推算值配上了对照（实测远低于上界）。
- 5 个新用例（`VideoWallLoopbackTests`）会在真实连接上核对：每个节点按 `clear → mapping → commit → refresh` 收齐 4 个包且**逐字节**等于构造结果；双屏模式从收到的字节里解出组播地址、窗口号与源矩形，按列坐标核对左右半墙（左 5 列 `224.1.1.55`/窗口 1、右 5 列 `224.1.1.56`/窗口 2）；故障时其余 49 个节点**只收到清屏包**（不会提交半套映射）；失败日志点名到 `IP:端口 phase=…`。
- 需要管理员与机器准备，因此都带 `Physical` trait：缺别名时按各自前置条件跳过并在跳过原因里写出准备命令，常规命令 `--filter "Category!=Physical"` 仍是 **219/219**，不受影响。别名在本轮结束时已全部归还（`-Clear` 后 `-Status` 为 0/50）。
- **回环不等于现场**：假节点收到的是自己发的包，回声不能证明帧内容对真实墙面正确。issue #2 的抓包复核、SC-007 的物理画面、TCP_NODELAY、真实网段的时延与丢包都仍在外面，边界写进了 `specs/004-video-wall-control/verification.md` 与 `docs/known-pitfalls.md` 坑 9。本轮**未改动任何产品代码**。

### 视频墙控制包补上逐包黄金样本比对

- 视频墙的包级测试此前只断言单屏模式**第一个**映射包的字节，**双屏模式 50 个映射包一个字节都没有断言**：把右半墙的组播地址与窗口号错写成左半墙的值，旧断言的 4 个用例**全部通过**。现补上 `VideoWallGoldenPacketsTests`——两种模式各 200 个包，与旧 Python 实现**实际运行结果**导出的黄金样本逐项比对（阶段、`IP:端口`、包字节），逐节点的裁切区、组播地址、窗口号与校验和都进了断言。
- 样本由 `tools/generate_video_wall_golden.py` 生成（落在 `runtime-dotnet/tests/ScpCv.Infrastructure.Tests/Fixtures/video-wall-packets.json`），并由 `tests/test_video_wall_golden_fixture.py` 守在样本这一头：样本一旦与 `scp_cv/services/video_wall.py` 脱节，.NET 侧的比对就退化成「拿旧结果对新代码」，这条静默路径有测试封住。
- 验证：新增 3 个 .NET 用例与 3 个 Python 用例，.NET 全量 **219/219** 通过（原 216）；本轮**未改动任何产品代码**。探针分两批——右半墙组播地址/窗口号写错（新用例报「第 75 个控制包与旧实现不一致」，旧用例仍全绿）、样本第 75 项改错（Python 用例报「At index 75 diff」），撤销后逐字节还原。
- 仍未解决：issue #2 的**帧内容现场抓包复核**。仓库里的固定包与映射参数只追溯到旧 Python 实现，没有厂商协议文档、没有抓包记录；本机跑假节点收到的是自己发的包，回声不构成证据。`specs/004-video-wall-control/verification.md` 末节补上了这条债的承接方与「补完改哪一行」，并记下一个此前没人注意到的事实：`docs/实机测试结论.md:39` 用 RGB 测试卡物理确认过**双屏**模式的半区映射，而 `:41` 的「单屏/双屏切换 通过」核的全是 UI/运行态——**单屏整墙至今没有任何现场画面证据**。

## 2026-09-19

### 修复切换大屏模式不下发拼接屏控制包

- 补齐缺失的视频墙下发层：清屏 → 映射 → 提交 → 刷新，阶段内并发上限 25、阶段间串行、单节点 2 秒超时、开启 TCP_NODELAY、失败退避重试 5 次。任一阶段有节点未收到即中止后续阶段，不再提交半套映射。
- `PATCH /api/runtime/` 与预案激活在切换大屏模式时真正下发控制包；下发失败时运行态、静音策略与整个预案激活都不生效，接口返回 `video_wall_error`，避免出现“界面已切双屏、墙上仍是旧画面”。
- Simulation 模式保持不发网络包（与显示器拓扑、系统音频的既有分工一致），现场以 `-SafetyMode Hardware` 启动时才走真实 TCP。
- 验证：新增 11 个测试（下发时序、重试次数、失败即中止、Hardware/Simulation 装配、两条入口的下发与不回滚）；移除下发调用后新测试全部失败；.NET 全量 208 个测试通过。
- 规范：补齐 `specs/004-video-wall-control/`（节点清单与协议阶段、并发与重试约束、失败即整体不生效、Simulation/Hardware 分工、现场验证边界）。此前的 `specs/` 从未出现视频墙条目，这正是下发层被静默漏迁的原因；物理画面（SC-007）在取得现场证据前标注为未实机验证。
- 验证记录见 `specs/004-video-wall-control/verification.md`。

### 补齐大屏模式的下发日志与文档

- 视频墙下发补齐日志（FR-018）：成功记录模式与控制包数，失败记录中止阶段、失败节点数与首个原因，节点重试成功记录 `IP:端口` 与第几次尝试，取消既不记失败也不记成功；Simulation 记录「跳过下发、未建立连接」。此前成功与失败都没有任何日志，失败只体现在 HTTP 错误体里——节点只接受写入、不回读状态，日志是现场判断“墙面到底动没动”的唯一线索。
- `docs/使用文档.md` §4.5「大屏模式」补充下发语义：整段下发成功才更新运行态与静音策略、失败时接口返回 `video_wall_error`（预案为 `scenario_error`）且界面保持原模式、正常 3 秒返回/最坏接近 90 秒、Simulation 下切换成功不代表墙面会动作，并标注物理画面（SC-007）未实机验证（FR-020）。
- `docs/维护文档.md` 新增 §8.6「切换大屏模式失败或墙面不动作」，按日志条目给出定位顺序；§8.5 增加与设备电源的区分提示（两者都叫“拼接屏”，链路不同）。
- 验证：新增 5 个日志测试；把日志改为不可达后 4 个新增测试全部失败（第 5 个断言“不记日志”，故不受该探针影响），撤销后源码与探针前逐字节一致（md5 `a0a61b44`）；.NET 全量 213 个测试通过（原 208）。

### 补齐 FR-012/FR-013 的自动化断言

- 重复下发当前已生效的大屏模式（切换报成功但墙面没动时的现场补救路径）现在有测试保证会完整重发：服务层证明 `SetRuntimeModeAsync` 没有相同模式早退，控制器层证明 `DispatchAsync` 不去重——同模式连发两次会发出 400 个控制包，200 个「节点 × 阶段」各尝试 2 次。
- 这条补救路径此前只存在于代码里（前端 `selectMode` 刻意不短路、后端同理），文档从未告诉现场怎么用：`docs/使用文档.md` §4.5 与 `docs/维护文档.md` §8.6 补上「切换成功但墙面没动时再点一次当前模式」及其判断依据。
- 并发下发串行化现在有测试保证：在第一次下发卡在映射阶段时发起第二次，第二次一个包都发不出去；释放后整段折叠为两次完整的「清屏 → 等待 → 映射 → 提交 → 刷新」序列。此前 `_gate` 只有代码复核，没有任何并发验证。
- 验证：新增 3 个测试，.NET 全量 216/216 通过（原 213）。两批探针分别让对应用例失败——控制器加 `_lastMode` 去重 + 放开 `_gate`（2 个用例失败），服务层加同模式短路（1 个用例失败）；探针撤销后源码逐字节一致，本次**未改动任何产品代码**。
...
- 无头启动支持多 Origin、精确 Host 白名单与 `0.0.0.0` 局域网 HTTP 监听，开发口令改由进程环境或 ACL 私有文件传递；D2 防火墙只允许私网 TCP 18443，且未启动 Worker。

## 2026-09-07

### 播放控制能力与实际模式展示修复

- 显控页按后端真实能力禁用 PPT/PDF、图片、网页等不支持类型的窗口音量与静音，视频和三类直播流保持可用。
- `PlaybackSession` 持久化播放器实际采用的 PowerPoint/PDF 适配器；会话快照不再用媒体源首选模式冒充实际模式，COM 槽位冲突回退后前端会显示“PDF 模式”。
- PowerPoint 重置仅作用于实际使用 COM 的窗口，不会重开已经回退 PDF 的窗口。
- PPT 资源请求增加源 ID 与请求序号守卫，快速切源时过期响应或错误不会覆盖当前缩略图与媒体列表。
- 增加前端能力、服务快照、控制器上报、迁移和 REST 回归测试，并完成桌面与手机活跃播放态浏览器复核。

## 2026-09-06

### 前端展示与移动交互回归修复

- 按远端规范完成代码整合，本地 Broker 改造保留在历史和备份中，不进入当前运行架构。
- 修复仪表盘状态标签未注册、手机“更多”误跳首页、源与预案抽屉超出视口、显控空闲提示重复。
- 修复平板媒体表格操作列被裁切和侧栏被挤窄，约束长名称卡片标题宽度。
- 增加 `pnpm --prefix frontend test` 前端回归检查，记录浏览器范围、验证结果和实机边界于 `docs/前端检查结果.md`。
...
- PDF 播放器：由 `QPdfView` 改为 `QPdfDocument.render()` + 自绘 QLabel，消除灰框、滚动和页面留白。
- 专注模式：上一页/下一页在 PDF 与 PowerPoint 模式都显示；移除两处“无页面媒体控制”提示；“演示文稿专注模式”改“专注模式”并支持窄屏换行。
- 显示控制：当前输出缩略图跟随演示文稿当前页；媒体源列表仍使用第一页缩略图；缺失的演示文稿缩略图会懒修复。
- 上传区：删除冗长 `n-form-item` feedback，仅保留“演示文稿推荐使用 PDF 格式上传”提示；SourcePicker 增加“仅上传”按钮；摘要改为“上传新文件”；上传完成后进入“处理中”状态。
- 前端稳定性：升级到当前最新版本（Vite 8.2、vue-router 5.2、pinia 4.0.2、Vue 3.5.41 等），TypeScript 使用最新 6.x（6.0.3）以兼容 vue-tsc 3.3.9；runall 对 Vue 前端异常退出增加最多 3 次自动重启；网络断开时返回明确的中文错误提示。
- 验证：后端全量 `383 passed`，Django check、Ruff、前端 typecheck 与生产构建通过。

### 多轮浏览器 QA

- 新增 `/api/auth/status/` 公开登录态探测接口，前端首屏改用它，消除未登录时的 401 控制台噪声。
- 修复媒体源搜索无结果时的空态文案。
- 完成常规、异常、极限与并发四轮 Playwright 真人式测试；验证登录、媒体源、显控、PDF 翻页/缩略图/专注模式、背景音乐、预案、设置、移动端、注销及不可达网页。
- 验证：后端全量 `385 passed`，Django check、Ruff、前端 typecheck 与生产构建通过。

### 演示文稿 PDF 播放与 PowerPoint 单槽位

- 媒体源：接受 `.pdf` 作为演示文稿源；新增静态内容检测（动画、切换、音视频、嵌入对象）与自动 PDF 导出；直接上传 PDF 无需转换。
- 播放器：新增 QtPdf 演示文稿适配器，静态 PPT/PDF 播放不再启动 PowerPoint；动态演示文稿仍走 PowerPoint，且同一时刻只允许一份完整放映，切换前完整关闭旧放映并等待 COM 释放。
- 提词器：OOXML 备注继续写入 `PptResource.speaker_notes`，PDF 播放时专注模式同样显示提词器。
- 预热：本轮停用 PowerPoint 预热，保留预热池扩展入口；网页预热和后续统一预热模型留待后续任务。
- 前端：演示文稿文案统一，上传提示“若提交的是演示文稿且无动画或媒体资源，建议使用 PDF 提交”；PDF 模式隐藏页面媒体控制并显示模式徽标。
- 验证：后端全量 `383 passed`，Django check、Ruff、前端 typecheck 与生产构建通过。
...
- 播放器：接管 PowerPoint 应用级预热代理前验证 `Application.Presentations`；另一窗口仍在放映导致代理失效时，自动退休旧代理并通过 `DispatchEx` 重新连接。
- 测试：新增失效预热 Application 自动重连回归；两个真实前端页面完成不同 PPT 同时放映、独立翻页、单侧关闭/重开/切换及最终黑场验证。
- 验证：后端全量 `370 passed`，Django check、Ruff 通过；D2 摄像机确认左右独立输出和稳定清场。

## 2026-07-19

### 完整用户流程回归后的视觉修复

- 背景音乐：修正 hero 大标题小于字号的行高，避免中文 glyph 在 `overflow: hidden` 容器中被裁掉上沿。
- 设置：REST 与 Vite 端口摘要改为读取实际 `VITE_BACKEND_TARGET` 和浏览器 origin；使用自定义 `runall --backend-port/--frontend-port` 时不再固定显示 `8000/5173`。
- 验证：通过登录、媒体源增删改查、四类切源、临时上传、PPT 翻页/跳页/页内媒体、背景音乐、预案、设置、移动端与退出鉴权共 14 条浏览器用户流程。

## 2026-07-18

### 修复显控、PPT 操作与前端可用性

- 播控：新增数据库持久指令队列，连续翻页按序交付，高频 Seek/音量/静音/循环合并为最新值，OPEN/CLOSE/RESET_PPT 自动淘汰旧画面的积压指令；兼容字段仍保留给旧调用方。
- 切源：新源打开失败并恢复旧 adapter 时同步回滚会话 source，避免前端显示新源、物理画面仍是旧源；不可用源禁止点击和下发打开命令。
- 在线状态：播放器按窗口上报心跳，顶栏与首页将 SSE“控制链路”和真实播放器在线数分开显示，避免只有后端在线时误报“实时已连接”。
- 媒体源：切换面板与媒体库标出正在播出的窗口；移动端切源成功后自动进入控制面板；显控页增加当前输出参考预览，并明确它不是物理屏回传画面；“上传并打开”改为临时源，媒体库上传只保留持久保存语义。
- PPT/视频：PPT 专注模式在窄屏保留当前页并增加停止当前页媒体按钮；普通控制页不再每次翻页都重新请求整份 PPT 资源；视频 Seek 改为节流提交并在拖动结束时刷新最终位置。
- 安全与测试：设置页新增当前用户修改密码入口，登录页不再公开默认口令；shutdown API 测试使用临时信号目录，不再意外停止本机运行中的 `runall`。
...
- 前端：PPT 专注模式在电脑横屏布局下新增左侧页面缩略图栏，支持按页预览、当前页高亮和点击直接跳转。
